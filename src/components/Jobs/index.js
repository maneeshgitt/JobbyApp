...
class Jobs extends Component {
    ...
    onGetInputOption = event => {
        ...
        if(inputNotInList.length === 0){
            ...
            this.setState(
                {checkboxInputs : filteredData}, this.onGetJobDetails,
            )
        }
    }
    ...
    onGetJobsFailureView = () => (
        <div className="failure-img-button-container">
            ...
            <div className="jobs-failure-button-container">
                <button className="failure-button" type = "button" onClick = {this.onRetryJobs}>retry</button>
            </div>
        </div>
    )
        ...
        <ul className="check-boxes-container">
            {employmentTypesList.map(eachItem => (
                <li className="li-container" key = {eachItem.employmentTypeId}>
                    <input className="jobs-input" id = {eachItem.employmentTypeId} type = "checkbox" onChange={this.onGetInputOption} />
                    <label className="jobs-label" htmlFor = {eachItem.employmentTypeId} >{eachItem.label}</label>
                </li>
            ))}
        </ul>
    )
        ...
        <ul className="radio-button-container">
            {salaryRangesList.map(eachItem => (
                <li className="li-container" key = {eachItem.salaryRangeId}>
                    <input className="radio" id = {eachItem.salaryRangeId} type = "radio" name = "option" onChange = {this.onGetRadioOption} />
                    <label className="label" htmlFor = {eachItem.salaryRangeId}>{eachItem.label}</label>
                </li>
            ))}
        </ul>
    )
            ...
            <>
                ...
                <div className="all-jobs-container">
                    ...
                    <div className="jobs-container">
                        <div>
                            <input className="jobs-search-input" type = "search" value = {searchInput} placeholder="Search" onChange={this.onGetSearchInput} onKeyDown = {this.onEnterSearchInput} />
